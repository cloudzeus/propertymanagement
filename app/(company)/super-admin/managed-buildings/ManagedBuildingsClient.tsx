...
function StatusPill({ status }: { status: ObligationStatus }) {
  ...
  return <span style={{ display: "inline-flex", alignItems: "center", fontSize: "var(--fs-11-5)", fontWeight: 700, padding: "3px 9px", borderRadius: 999, background: s.bg, color: s.fg, whiteSpace: "nowrap" }}>{s.label}</span>;
}
  ...
  const columns: ColDef<ManagedBuildingRow>[] = [
    { id: "name", header: "Κτήριο", accessor: (b) => b.name, cell: (b) => (
      <div>
        ...
        <div style={{ fontSize: "var(--fs-11-5)", color: "var(--muted-foreground)" }}>{b.customerName}{b.address ? ` · ${b.address}` : ""}</div>
      </div>
    ) },
  ...
  return (
    <div style={{ padding: "22px 24px 40px", maxWidth: 1240 }}>
      <header style={{ marginBottom: 18 }}>
        <h1 style={{ margin: 0, fontSize: "var(--fs-22)", fontWeight: 800, letterSpacing: "-.01em", color: "var(--foreground)", display: "flex", alignItems: "center", gap: 9 }}>
          <RiBuilding2Line /> Διαχειριζόμενα κτήρια
        </h1>
        <p style={{ margin: "4px 0 0", fontSize: "var(--fs-13-5)", color: "var(--muted-foreground)" }}>Στοιχεία, προγράμματα συντήρησης και υποχρεώσεις των κτηρίων που διαχειρίζεται η εταιρεία.</p>
        <button onClick={() => setAssignOpen(true)} style={{ marginTop: 12, display: "inline-flex", alignItems: "center", gap: 7, border: "none", background: "var(--color-primary)", color: "#fff", borderRadius: 6, padding: "8px 14px", fontSize: "var(--fs-13)", fontWeight: 600, cursor: "pointer" }}>
  ...
  return (
    <div style={{ padding: "12px 16px", display: "grid", gridTemplateColumns: "1fr 1fr", gap: 20 }}>
      ...
      <div style={{ gridColumn: "1 / -1" }}>
        <Link href={`/super-admin/buildings/${building.id}`} style={{ display: "inline-flex", alignItems: "center", gap: 6, fontSize: "var(--fs-13)", fontWeight: 700, color: "var(--color-primary)", textDecoration: "none" }}>
...
function StatCard({ icon, tint, value, label }: { icon: React.ReactNode; tint: string; value: number; label: string }) {
  return (
    <div style={{ display: "flex", alignItems: "center", gap: 12, background: "var(--card)", border: "1px solid var(--border)", borderRadius: 12, padding: "14px 16px" }}>
      ...
      <div>
        <div style={{ fontSize: "var(--fs-20)", fontWeight: 800, lineHeight: 1.1, color: "var(--foreground)", fontVariantNumeric: "tabular-nums" }}>{value}</div>
        <div style={{ fontSize: "var(--fs-12)", color: "var(--muted-foreground)", marginTop: 1 }}>{label}</div>
      </div>
    </div>
  );
}
...
const panelTitle: React.CSSProperties = { display: "flex", alignItems: "center", gap: 8, fontSize: "var(--fs-14)", fontWeight: 700, color: "var(--foreground)", marginBottom: 10 };
const sectionTitle: React.CSSProperties = { display: "flex", alignItems: "center", gap: 7, fontSize: "var(--fs-12-5)", fontWeight: 700, color: "var(--foreground)", marginBottom: 8 };
const rowLine: React.CSSProperties = { display: "flex", alignItems: "center", gap: 6, flexWrap: "wrap", fontSize: "var(--fs-13)", color: "var(--foreground)", padding: "6px 0", borderTop: "1px solid var(--border)" };
const muted: React.CSSProperties = { fontSize: "var(--fs-12-5)", color: "var(--muted-foreground)", padding: "6px 0" };
...
function AssignItemModal({ buildings, itemTypes, onClose }: { buildings: ManagedBuildingRow[]; itemTypes: { id: string; name: string }[]; onClose: () => void }) {
  ...
  return (
    <Modal open onClose={onClose} title="Ανάθεση διαχειριζόμενου στοιχείου" width={520}
      ...
      {error && <div style={{ padding: "8px 12px", borderRadius: 6, background: "#fee2e218", color: "#dc2626", fontSize: "var(--fs-12)", border: "1px solid #fca5a530", marginBottom: 12 }} role="alert">{error}</div>}
      <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
        ...
        <FormField label="Τοποθεσία" required>
          <input value={form.location} onChange={(e) => setForm((p) => ({ ...p, location: e.target.value }))} placeholder="π.χ. Κοινόχρηστοι χώροι" style={{ width: "100%", height: 36, padding: "0 10px", border: "1px solid var(--border)", borderRadius: "var(--radius-sm)", fontSize: "var(--fs-13)", color: "var(--foreground)", background: "var(--card)", outline: "none", boxSizing: "border-box" }} />
        </FormField>
        <FormField label="Ποσότητα" required>
          <input type="number" min={1} value={form.quantity} onChange={(e) => setForm((p) => ({ ...p, quantity: Math.max(1, Math.round(Number(e.target.value)) || 1) }))} style={{ width: 120, height: 36, padding: "0 10px", border: "1px solid var(--border)", borderRadius: "var(--radius-sm)", fontSize: "var(--fs-13)", color: "var(--foreground)", background: "var(--card)", outline: "none", boxSizing: "border-box", textAlign: "center", fontVariantNumeric: "tabular-nums" }} />
        </FormField>
          ...
          {sched.enabled && (
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10, marginTop: 10 }}>
              ...
              <FormField label="Επόμενη ημερομηνία">
                <input type="date" value={sched.nextDueDate} onChange={(e) => setSched((p) => ({ ...p, nextDueDate: e.target.value }))} style={{ width: "100%", height: 36, padding: "0 10px", border: "1px solid var(--border)", borderRadius: "var(--radius-sm)", fontSize: "var(--fs-13)", color: "var(--foreground)", background: "var(--card)", outline: "none", boxSizing: "border-box" }} />
              </FormField>
            </div>
          )}
        </div>
