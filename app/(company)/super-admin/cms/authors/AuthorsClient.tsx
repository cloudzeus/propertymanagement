  ...
  const columns: ColDef<Author>[] = [
    {
      ...
      accessor: (a) => a.name,
      cell: (a) => (
        <span style={{ fontSize: "var(--fs-13)", fontWeight: 600, color: "var(--foreground)" }}>{a.name}</span>
      ),
    },
    {
      id: "slug", header: "Slug", sortKey: "slug", width: 240,
      accessor: (a) => a.slug,
      cell: (a) => <span style={{ fontSize: "var(--fs-13)", color: "var(--muted-foreground)" }}>{a.slug || "—"}</span>,
    },
  ];
