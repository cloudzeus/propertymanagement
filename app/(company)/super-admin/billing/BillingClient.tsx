...
export function BillingClient({ rows, stats }: {
  ...
  const columns: ColDef<InvoiceRow>[] = [
    {
      ...
      cell: (r) => <span style={{ fontSize: "var(--fs-13)", fontWeight: 600, color: "var(--foreground)" }}>{r.customerName}</span>,
    },
    {
      id: "period", header: "Περίοδος", sortKey: "period", width: 110,
      accessor: (r) => r.period,
      cell: (r) => <span style={{ fontSize: "var(--fs-13)", color: "var(--muted-foreground)", fontFamily: "monospace" }}>{r.period}</span>,
    },
    {
      id: "amount", header: "Ποσό (€)", sortKey: "amount", width: 120,
      accessor: (r) => r.amount,
      cell: (r) => <span style={{ fontSize: "var(--fs-13)", fontWeight: 600, color: "var(--foreground)" }}>€ {r.amount.toFixed(2)}</span>,
    },
    {
      ...
      cell: (r) => {
        ...
        return <span style={{ fontSize: "var(--fs-11)", fontWeight: 600, padding: "2px 8px", borderRadius: 20, background: `${s.color}18`, color: s.color }}>{s.label}</span>;
      },
    },
    {
      id: "issuedAt", header: "Έκδοση", sortKey: "issuedAt", width: 120,
      accessor: (r) => r.issuedAt,
      cell: (r) => <span style={{ fontSize: "var(--fs-12)", color: "var(--muted-foreground)" }}>{r.issuedAt ? new Date(r.issuedAt).toLocaleDateString("el-GR") : "—"}</span>,
    },
    {
      id: "dueDate", header: "Λήξη", sortKey: "dueDate", width: 120, defaultVisible: false,
      accessor: (r) => r.dueDate ?? "",
      cell: (r) => <span style={{ fontSize: "var(--fs-12)", color: "var(--muted-foreground)" }}>{r.dueDate ? new Date(r.dueDate).toLocaleDateString("el-GR") : "—"}</span>,
    },
  ];
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 20 }}>
      <div>
        <h1 style={{ fontSize: "var(--fs-22)", fontWeight: 700, color: "var(--foreground)", margin: 0 }}>Τιμολόγηση</h1>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(4, 1fr)", gap: 16 }}>
        {cards.map((c) => (
          <div key={c.label} style={{ background: "var(--card)", border: "1px solid var(--border)", borderRadius: "var(--radius)", padding: "18px 20px" }}>
            <div style={{ fontSize: "var(--fs-12)", color: "var(--muted-foreground)", fontWeight: 500, marginBottom: 8 }}>{c.label}</div>
            <div style={{ fontSize: "var(--fs-24)", fontWeight: 700, color: c.color }}>{c.value}</div>
          </div>
        ))}
      </div>
      ...
      {rows.length === 0 ? (
        <div style={{ background: "var(--card)", border: "1px solid var(--border)", borderRadius: "var(--radius)", padding: "48px 24px", textAlign: "center", color: "var(--muted-foreground)" }}>
          <RiFileList3Line style={{ fontSize: "var(--fs-40)", opacity: 0.4, display: "block", margin: "0 auto 12px" }} />
          <p style={{ fontSize: "var(--fs-14)", fontWeight: 600, color: "var(--foreground)", margin: "0 0 6px" }}>Δεν υπάρχουν τιμολόγια ακόμη</p>
          <p style={{ fontSize: "var(--fs-13)", margin: 0, maxWidth: 460, marginInline: "auto", lineHeight: 1.5 }}>
