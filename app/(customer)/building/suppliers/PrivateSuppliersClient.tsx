  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 18 }}>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", gap: 12, flexWrap: "wrap" }}>
        <div>
          <h1 style={{ display: "flex", alignItems: "center", gap: 10, fontSize: 24, fontWeight: 800, margin: 0, color: "var(--foreground)" }}>
      ...
      {platform && (
        <div style={{ ...card, display: "flex", alignItems: "center", gap: 14, borderColor: "var(--color-primary)55", background: "var(--color-primary)08" }}>
          <div style={{ width: 44, height: 44, borderRadius: 10, background: "var(--color-primary)18", color: "var(--color-primary)", display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0 }}>
            <RiBuilding4Line style={{ fontSize: 24 }} />
          </div>
          <div style={{ flex: 1, minWidth: 0 }}>
            <div style={{ fontSize: 15, fontWeight: 800, color: "var(--foreground)" }}>{platform.name} <span style={pill}>Προεπιλογή</span></div>
      ...
      <div style={{ position: "relative", maxWidth: 380 }}>
        ...
        <input value={q} onChange={(e) => setQ(e.target.value)} placeholder="Αναζήτηση (επωνυμία, ΑΦΜ, ειδικότητα, πόλη)…"
          style={{ width: "100%", height: 36, padding: "0 10px 0 32px", border: "1px solid var(--border)", borderRadius: "var(--radius-sm)", fontSize: 13, background: "var(--card)", color: "var(--foreground)", boxSizing: "border-box" }} />
      </div>

      {mine.length === 0 ? (
        <div onClick={() => caps.create && setEditing("new")} style={{ border: "1.5px dashed var(--border-strong)", borderRadius: 8, padding: 40, textAlign: "center", color: "var(--muted-foreground)", cursor: caps.create ? "pointer" : "default", background: "var(--bg-canvas)", fontSize: 13 }}>
          <RiTruckLine style={{ fontSize: 26, display: "block", margin: "0 auto 8px" }} />
          ...
          {mine.map((s) => (
            <div key={s.id} style={{ ...card, opacity: s.isActive ? 1 : 0.6 }}>
              <div style={{ display: "flex", alignItems: "flex-start", gap: 8 }}>
                <div style={{ flex: 1, minWidth: 0 }}>
                  <div style={{ fontSize: 15, fontWeight: 800, color: "var(--foreground)", display: "flex", alignItems: "center", gap: 6, flexWrap: "wrap" }}>
                    {s.name}
                    {!s.isActive && <RiEyeOffLine title="Ανενεργός" style={{ fontSize: 13, color: "var(--muted-foreground)" }} />}
                    {s.emergency24h && <RiAlarmWarningLine title="Έκτακτα 24/7" style={{ fontSize: 13, color: "#c50f1f" }} />}
                  </div>
                  <div style={{ fontSize: 12, color: "var(--muted-foreground)" }}>{SUPPLIER_KIND_LABELS[s.kind]}{s.afm ? ` · ΑΦΜ ${s.afm}` : ""}</div>
                </div>
              ...
              <div style={{ display: "flex", flexDirection: "column", gap: 4, marginTop: 10, fontSize: 12.5, color: "var(--foreground)" }}>
...
const pill: React.CSSProperties = { fontSize: 10.5, fontWeight: 700, padding: "1px 7px", borderRadius: 9999, background: "var(--color-primary)", color: "#fff", marginLeft: 6, verticalAlign: "middle" };
