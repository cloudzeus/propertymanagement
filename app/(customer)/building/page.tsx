...
export default async function ManagerBuildingsPage({
  ...
  let ids = await managerBuildingIds(userId);
  // No management assignments: fall back to buildings the user occupies as
  // owner/resident/open-occupancy (see getBuildingAccess's occupant branch).
  const isOccupant = ids.length === 0;
  if (isOccupant) {
    const units = await db.unit.findMany({
      where: { OR: [{ ownerId: userId }, { residentId: userId }, { occupancies: { some: { userId, endDate: null } } }] },
      select: { buildingId: true },
    });
    ids = [...new Set(units.map((u) => u.buildingId))];
  }
  ...
  // Collection/unpaid figures span every resident's payments, not just the viewer's —
  // occupants only ever see their own building(s), never those money totals.
  const stats = new Map<string, { total: number; paid: number; unpaid: number }>();
  const openCounts = new Map<string, number>();
  if (!isOccupant) {
    const month = currentMonth();
    const [allocations, openGroups] = await Promise.all([
    ...
    for (const g of openGroups) openCounts.set(g.buildingId, g._count._all);
  }
