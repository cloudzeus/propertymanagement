...
export function OnboardingWizard({ supplier, catalog, categories, existing, userName }: {
  ...
  return (
    <div className="dash-page" style={{ maxWidth: 860, margin: "0 auto", display: "flex", flexDirection: "column", gap: 18 }}>
      <div>
        <div style={{ fontSize: "var(--fs-12)", fontWeight: 700, letterSpacing: 0.5, textTransform: "uppercase", color: "var(--muted-foreground)" }}>Πρώτη σύνδεση</div>
        <h1 style={{ fontSize: "var(--fs-24)", fontWeight: 800, margin: "4px 0 0", color: "var(--foreground)" }}>Καλώς ήρθατε{userName ? `, ${userName}` : ""}</h1>
        <p style={{ margin: "6px 0 0", fontSize: "var(--fs-13-5)", color: "var(--muted-foreground)", maxWidth: 620 }}>
      ...
      <ol style={{ listStyle: "none", margin: 0, padding: 0, display: "grid", gridTemplateColumns: "repeat(4, 1fr)", gap: 6 }}>
        {STEPS.map((s, i) => {
          ...
          return (
            <li key={s.key} style={{ display: "flex", alignItems: "center", gap: 8, padding: "10px 12px", borderRadius: 8, fontSize: "var(--fs-12-5)", fontWeight: 600,
              ...
              <span style={{ width: 22, height: 22, borderRadius: 999, display: "inline-flex", alignItems: "center", justifyContent: "center", background: state === "done" ? "#15803d" : "transparent", color: state === "done" ? "#fff" : "inherit", border: state === "done" ? "none" : "1px solid currentColor", fontSize: "var(--fs-11)" }}>
      ...
      <div style={{ background: "var(--card)", border: "1px solid var(--border)", borderRadius: 10, padding: 20 }}>
        ...
        {step === 1 && (
          <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
            <p style={{ margin: 0, fontSize: "var(--fs-13)", color: "var(--muted-foreground)" }}>
            ...
            {catalog.length === 0 && <div style={{ fontSize: "var(--fs-13)", color: "var(--muted-foreground)" }}>Η εταιρεία διαχείρισης δεν έχει ανοίξει ακόμη υπηρεσίες — προχωρήστε στο επόμενο βήμα.</div>}
            {grouped.map(([cat, items]) => (
              <div key={cat}>
                <div style={{ fontSize: "var(--fs-12)", fontWeight: 700, color: "var(--muted-foreground)", textTransform: "uppercase", letterSpacing: 0.4, marginBottom: 6 }}>{cat}</div>
                    ...
                    return (
                      <div key={c.id} style={{ display: "grid", gridTemplateColumns: "minmax(0,1fr) 120px 120px", gap: 8, alignItems: "center", padding: "8px 10px", borderRadius: 8, border: `1px solid ${on ? "var(--color-primary)" : "var(--border)"}`, background: on ? "var(--color-primary)08" : "var(--paper)" }}>
                        <label style={{ display: "flex", alignItems: "flex-start", gap: 8, cursor: "pointer", minWidth: 0 }}>
                          ...
                          <span style={{ minWidth: 0 }}><b style={{ fontSize: "var(--fs-13-5)" }}>{c.name}</b>{c.description && <div style={{ fontSize: "var(--fs-12)", color: "var(--muted-foreground)" }}>{c.description}</div>}</span>
                        </label>
        ...
        {step === 2 && (
          <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
            <p style={{ margin: 0, fontSize: "var(--fs-13)", color: "var(--muted-foreground)" }}>Πότε δέχεστε εργασίες; Το ωράριο εμφανίζεται στην εταιρεία διαχείρισης όταν επιλέγει συνεργάτη.</p>
        ...
        {step === 3 && (
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 14, fontSize: "var(--fs-13-5)" }}>
            ...
            <div style={{ background: "var(--paper)", borderRadius: 8, padding: 14, fontSize: "var(--fs-12-5)", color: "var(--muted-foreground)" }}>
...
function Summary({ title, rows }: { title: string; rows: [string, string][] }) {
  return (
    <div style={{ border: "1px solid var(--border)", borderRadius: 8, padding: "10px 14px" }}>
      <div style={{ fontSize: "var(--fs-12)", fontWeight: 700, color: "var(--muted-foreground)", textTransform: "uppercase", letterSpacing: 0.4, marginBottom: 6 }}>{title}</div>
...
const smallInput: React.CSSProperties = { height: 32, padding: "0 8px", border: "1px solid var(--border)", borderRadius: "var(--radius-sm)", fontSize: "var(--fs-12-5)", color: "var(--foreground)", background: "var(--card)", width: "100%", boxSizing: "border-box" };
