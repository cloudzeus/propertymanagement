      ...
      <div className="mt-[34px] inline-flex gap-1.5 rounded-[12px] bg-[rgba(27,28,26,.05)] p-1">
        {[
          ...
          <button
            ...
            onClick={seg.set}
            aria-pressed={seg.on}
            className={`rounded-[9px] px-5 py-[11px] text-[length:var(--fs-13-5)] transition-[background,box-shadow] duration-[180ms] ${
              seg.on
        ...
        {plans.map((p) => (
          ...
          >
            {p.highlighted && p.badge ? (
              <span className="u-caps absolute -top-3 left-[30px] rounded-full bg-[var(--accent)] px-3 py-1.5 text-[length:var(--fs-11)] font-extrabold tracking-[.09em] text-[#1b1c1a]">
                {p.badge}
              </span>
            ) : null}

            <h3 className="text-[length:var(--fs-20)] font-extrabold tracking-[-.01em]">{p.name}</h3>
            {/* min-height keeps the price line level across cards of different copy lengths */}
            <p className="mt-[7px] text-[length:var(--fs-13-5)] leading-[1.5] text-[var(--mut)]" style={{ minHeight: 40 }}>
              {p.description}
            </p>

            <div className="mb-1 mt-[22px] flex items-end gap-2">
              <span className="tnum text-[length:var(--fs-52)] font-extrabold leading-[.92] tracking-[-.035em]">
                {formatEuro(p.monthlyPrice * factor, locale, 2)}
              </span>
              <span className="pb-[5px] text-[length:var(--fs-12-5)] leading-[1.35] text-[var(--mut)]" style={{ maxWidth: 92 }}>
                {priceUnit}
              </span>
            </div>

            {p.minPerBuilding != null ? (
              <p className="tnum text-[length:var(--fs-12)] text-[var(--mut2)]">
            ...
            {p.features.length > 0 && (
              <ul className="mb-7 mt-[26px] flex flex-col gap-3">
                {p.features.map((feature, i) => (
                  <li key={i} className="flex items-start gap-[11px] text-[length:var(--fs-14)] leading-[1.5]">
                    <Tick />
                    <span>{feature}</span>
                  </li>
                ))}
