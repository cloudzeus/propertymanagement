...
export default async function Page() {
  ...
  const description =
    c.body.replace(/[#*_`>\-]/g, "").replace(/\s+/g, " ").trim().slice(0, 160) ||
    (locale === "el" ? "Υπηρεσίες διαχείρισης ακινήτων από την PropertyPro." : "Property management services by PropertyPro.");

  return (
    <div className="min-h-screen bg-white">
      <JsonLd
        data={[
          serviceSchema({ name: c.title, description, provider: "PropertyPro" }),
          breadcrumbSchema([
            { name: "Home", url: SITE_BASE },
            { name: c.title, url: `${SITE_BASE}${PATH}` },
          ]),
        ]}
      />
      <LandingHeader />
