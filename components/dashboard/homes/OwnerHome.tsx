...
/**
 * Owner dashboard content, scoped to `userId`. `userName` seeds the greeting.
 * `previewMode` skips realtime SSE wiring for the read-only role preview.
 * Rendered by the real /owner page and by the read-only role preview.
 */
export async function OwnerHome({ userId, userName, previewMode = false }: { userId: string; userName?: string | null; previewMode?: boolean }) {
  ...
  return (
    <div className="dash-page" style={{ display: "flex", flexDirection: "column", gap: 20 }}>
      {!previewMode && buildingIds.slice(0, 5).map((id) => (
