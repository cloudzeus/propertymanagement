...
export function Prose({ children }: { children: string }) {
  return (
    <div
      className={[
        "text-[length:var(--fs-17)] leading-[1.75] text-[var(--prose-ink)] sm:text-[length:var(--fs-18)]",
        "[&>p]:mb-[26px]",
        // Drop cap, first paragraph only.
        "[&>p:first-of-type]:dropcap",
        "[&_h2]:mb-[18px] [&_h2]:mt-12 [&_h2]:text-[length:var(--fs-24)] [&_h2]:font-extrabold [&_h2]:leading-[1.2] [&_h2]:tracking-[-.02em] [&_h2]:text-[var(--txt)] sm:[&_h2]:text-[length:var(--fs-29)]",
        "[&_h3]:mb-3 [&_h3]:mt-9 [&_h3]:text-[length:var(--fs-24)] [&_h3]:font-extrabold [&_h3]:tracking-[-.015em] [&_h3]:text-[var(--txt)]",
        // Lists: no bullets — an amber "›" marker does the work.
        "[&_ul]:mb-7 [&_ul]:flex [&_ul]:list-none [&_ul]:flex-col [&_ul]:gap-[13px] [&_ul]:p-0",
        "[&_ul>li]:relative [&_ul>li]:pl-6 [&_ul>li]:text-[length:var(--fs-17)] [&_ul>li]:leading-[1.6]",
        ...
        "[&_blockquote]:my-10 [&_blockquote]:border-l-[3px] [&_blockquote]:border-[var(--accent)] [&_blockquote]:py-1.5 [&_blockquote]:pl-7 [&_blockquote]:text-[length:var(--fs-21)] [&_blockquote]:font-semibold [&_blockquote]:leading-[1.42] [&_blockquote]:tracking-[-.012em] [&_blockquote]:text-[var(--txt)] sm:[&_blockquote]:text-[length:var(--fs-25)]",
        "[&_blockquote>p]:mb-0",
        ...
        "[&_table]:mb-7 [&_table]:w-full [&_table]:text-[length:var(--fs-15)] [&_th]:border-b [&_th]:border-[var(--line)] [&_th]:py-2.5 [&_th]:text-left [&_td]:border-b [&_td]:border-[var(--line2)] [&_td]:py-2.5",
        "[&_code]:rounded [&_code]:bg-[var(--paper)] [&_code]:px-1.5 [&_code]:py-0.5 [&_code]:text-[length:var(--fs-15)]",
      ].join(" ")}
    >
...
export function TakeawayBox({ label, items }: { label: string; items: string[] }) {
  ...
  return (
    <aside className="my-11 rounded-[18px] border border-[var(--line)] bg-[var(--paper)] px-8 py-[30px]">
      <div className="u-caps mb-[18px] text-[length:var(--fs-12)] font-extrabold tracking-[.12em] text-[var(--mut2)]">
        {label}
      </div>
      <ul className="flex flex-col gap-[13px]">
        {items.map((item, i) => (
          <li key={i} className="flex items-start gap-3.5 text-[length:var(--fs-17)] leading-[1.6]">
