...
export function PersonStatementModal({
  ...
  return (
    <Modal open={open} onClose={onClose} title={data ? `Καρτέλα — ${data.name}` : "Καρτέλα προσώπου"} width={860}>
      {error && <div style={{ padding: 10, borderRadius: 6, background: "color-mix(in srgb, var(--color-danger) 12%, transparent)", border: "1px solid var(--color-danger)", color: "var(--color-danger)", fontSize: "var(--fs-12)", marginBottom: 10 }}>{error}</div>}
      {!data ? (
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
          <div style={{ display: "flex", gap: 10, flexWrap: "wrap" }}>
            ...
            <div style={{ marginLeft: "auto", fontSize: "var(--fs-12)", color: "var(--muted-foreground)", alignSelf: "center" }}>{data.email ?? "— χωρίς email —"} · Μονάδες: {data.units.join(", ") || "—"}</div>
          </div>
                ...
                {data.lines.map((l) => {
                  ...
                  return (
                    <tr key={k} style={{ borderBottom: "1px solid var(--border)" }}>
                      ...
                      <td style={td}>{l.supplier ?? "—"}<div style={{ fontSize: "var(--fs-10)", color: "var(--muted-foreground)" }}>{fmtDate(l.documentDate)}{l.documentNumber ? " · " + l.documentNumber : ""}</div></td>
          ...
          {canPay && (
            <div style={{ display: "flex", alignItems: "center", justifyContent: "flex-end", gap: 10, borderTop: "1px solid var(--border)", paddingTop: 12 }}>
              <span style={{ fontSize: "var(--fs-12)", color: "var(--muted-foreground)" }}>{sel.size} επιλεγμένα</span>
              <select value={method} onChange={(e) => setMethod(e.target.value as PaymentMethod)} style={{ height: 34, padding: "0 10px", borderRadius: 6, border: "1px solid var(--border-strong)", background: "var(--bg-canvas)", fontSize: "var(--fs-13)" }}>
...
function Stat({ label, value, color }: { label: string; value: string; color?: string }) {
  return (
    <div style={{ border: "1px solid var(--border-strong)", borderRadius: 8, padding: "6px 12px", background: "var(--card)", minWidth: 100 }}>
      <div style={{ fontSize: "var(--fs-10)", color: "var(--muted-foreground)", fontWeight: 600 }}>{label}</div>
      <div style={{ fontSize: "var(--fs-15)", fontWeight: 800, color: color ?? "var(--foreground)" }}>{value}</div>
    </div>
  );
}

const th: React.CSSProperties = { padding: "7px 9px", fontWeight: 600, fontSize: "var(--fs-11)" };
...
const btnPay: React.CSSProperties = { display: "inline-flex", alignItems: "center", gap: 6, height: 34, padding: "0 16px", borderRadius: 6, border: "1px solid #16a34a", background: "#16a34a", color: "#fff", fontSize: "var(--fs-13)", fontWeight: 600, cursor: "pointer" };
