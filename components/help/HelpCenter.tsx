  ...
  return (
    <div className="dash-page" style={{ display: "grid", gridTemplateColumns: "240px minmax(0, 1fr)", gap: 20, alignItems: "start" }}>
      <aside style={{ position: "sticky", top: 16, display: "flex", flexDirection: "column", gap: 14 }}>
        <div>
          <div style={{ display: "flex", alignItems: "center", gap: 8, fontSize: 12, fontWeight: 700, color: "var(--muted-foreground)", textTransform: "uppercase", letterSpacing: 0.5, marginBottom: 8 }}>
          ...
          <nav style={{ display: "flex", flexDirection: "column", gap: 4 }}>
            {allowed.map((k) => {
              const on = k === key;
              return (
                <Link key={k} href={`${base}?m=${k}`} style={{ display: "block", padding: "9px 12px", borderRadius: 8, textDecoration: "none", fontSize: 13.5, fontWeight: on ? 700 : 500,
                  background: on ? "var(--color-primary)" : "var(--card)", color: on ? "#fff" : "var(--foreground)", border: `1px solid ${on ? "var(--color-primary)" : "var(--border)"}` }}>
                  {MANUALS[k].title}
                  <span style={{ display: "block", fontSize: 11, opacity: 0.8, fontWeight: 400 }}>{MANUALS[k].audience}</span>
                </Link>
              );
            })}
          </nav>
        </div>
        <div style={{ background: "var(--card)", border: "1px solid var(--border)", borderRadius: 10, padding: 14, fontSize: 13 }}>
      ...
      <article style={{ background: "var(--card)", border: "1px solid var(--border)", borderRadius: 12, padding: "24px 32px", maxWidth: 860, fontSize: 15, lineHeight: 1.65 }}>
