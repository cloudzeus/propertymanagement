      ...
      {WEEKDAYS.map((d) => {
        ...
        return (
          <div key={d.key} style={{ display: "grid", gridTemplateColumns: compact ? "88px 1fr" : "110px 1fr", alignItems: "center", gap: 8, minHeight: 32 }}>
            <label style={{ display: "flex", alignItems: "center", gap: 6, fontSize: "var(--fs-13)", color: on ? "var(--foreground)" : "var(--muted-foreground)", cursor: "pointer" }}>
            ...
            <div style={{ display: "flex", alignItems: "center", gap: 6, flexWrap: "wrap" }}>
              ...
              )) : <span style={{ fontSize: "var(--fs-12)", color: "var(--muted-foreground)" }}>Κλειστά</span>}
              {on && ranges.length < 2 && (
...
const timeInput: React.CSSProperties = { height: 30, padding: "0 6px", border: "1px solid var(--border)", borderRadius: "var(--radius-sm)", fontSize: "var(--fs-12-5)", color: "var(--foreground)", background: "var(--card)" };
...
const linkBtn: React.CSSProperties = { alignSelf: "flex-start", background: "none", border: "none", color: "var(--color-primary)", fontSize: "var(--fs-12-5)", cursor: "pointer", padding: 0, textDecoration: "underline" };
