...
export function GeoStatus({ fields }: { fields: Field[] }) {
  return (
    <CmsCard title="GEO — LocalBusiness">
      <p style={{ fontSize: "var(--fs-13)", color: "var(--muted-foreground)", marginTop: 0 }}>
        <RiMapPin2Line style={{ verticalAlign: "-2px" }} /> Το τοπικό schema εμφανίζεται σε όλες τις σελίδες από τις Ρυθμίσεις.
      </p>
      <div style={{ display: "flex", flexWrap: "wrap", gap: 10 }}>
        {fields.map((f) => (
          <span key={f.label} style={{ display: "inline-flex", alignItems: "center", gap: 6, fontSize: "var(--fs-13)", color: f.ok ? "var(--color-success)" : "var(--muted-foreground)" }}>
            {f.ok ? <RiCheckboxCircleFill size={15} /> : <RiCloseCircleLine size={15} />} {f.label}
          </span>
        ))}
      </div>
      <div style={{ marginTop: 12 }}>
        <Link href="/super-admin/cms/settings" style={{ fontSize: "var(--fs-13)", color: "var(--primary)", fontWeight: 600 }}>
