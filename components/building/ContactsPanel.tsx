  ...
  return (
    <div>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 14 }}>
        <div style={{ fontSize: "var(--fs-13)", color: "var(--muted-foreground)", display: "flex", alignItems: "center", gap: 6 }}><RiContactsBook3Line /> {contacts.length} επαφές</div>
            ...
            <tbody>
              {contacts.map((c) => (
                <tr key={c.id} style={{ borderTop: "1px solid var(--border)" }}>
                  <td style={td}><b>{c.name}</b>{c.notes && <div style={{ fontSize: "var(--fs-11)", color: "var(--muted-foreground)" }}>{c.notes}</div>}</td>
...
const chip: React.CSSProperties = { fontSize: "var(--fs-11)", fontWeight: 700, padding: "2px 8px", borderRadius: 9999, background: "var(--color-blue-soft)", color: "var(--color-blue)" };
