...
export interface EffectiveSession {
  user: { id: string; role: UserRole; companyId: string | null; customerId: string | null; name: string | null; email: string };
...
export async function getEffectiveSession(): Promise<EffectiveSession | null> {
  ...
  if (realRole === "SUPER_ADMIN") {
    ...
    if (imp && imp.actorId === realId) {
      ...
      if (target) {
        return {
          user: { id: target.id, role: target.role as UserRole, companyId: target.companyId ?? null, customerId: target.customerId ?? null, name: target.name, email: target.email },
  ...
  return {
    user: { id: realId, role: realRole, companyId: (session.user as any).companyId ?? null, customerId: (session.user as any).customerId ?? null, name: session.user.name ?? null, email: session.user.email ?? "" },
    real: { id: realId, role: realRole },
    impersonatorId: null,
  };
}
