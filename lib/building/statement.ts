...
export type UnitStatementInput = Omit<StatementExpense, "myShare" | "myTenant" | "myOwner"> & { unitAmount: number; unitTenant: number; unitOwner: number; receiptUrl?: string | null; description?: string | null };

export type UnitStatementGroup = {
  key: StatementGroupKey; label: string; buildingTotal: number;
  lines: { id: string; categoryName: string; amount: number; unitAmount: number; receiptUrl?: string | null; description?: string | null }[];
...
export function buildUnitStatement(unit: UnitStatementMeta, rows: UnitStatementInput[]): UnitStatement {
  ...
  for (const r of rows) {
    ...
    g.lines.push({ id: r.id, categoryName: r.categoryName, amount: r.amount, unitAmount: r.unitAmount, receiptUrl: r.receiptUrl ?? null, description: r.description ?? null });
